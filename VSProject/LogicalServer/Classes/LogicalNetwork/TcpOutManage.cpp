...
void TcpOutManage::_onTcpOutConnectResult(bool succeed, const FWNetMatch & netMatch, FW_ID custom)
{
	if (custom == Center)
	{
		if (succeed)
		{
			...
			loginReq.ServerID = LogicalMain::GetInstance()->GetLogicalID();
			Message::MsgPackageSend msgPackageSend(Message::Center::MSG_MAIN_LOGIN, Message::Center::MSG_ASS_LOGIN_LOGICAL, &loginReq);
